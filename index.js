...
con.connect(function(err) {
  if (err)
  {
    console.log("❌  Veuillez initialier la base de donnees (db.sql)" + err);
  }
...
app.get('/edit-profile', function(req, res){
  ...
  else
  {
    ...
    con.query(sql, function(err, result){
	  ...
	  con.query("SELECT DISTINCT(tags.name) FROM tags INNER JOIN user_tag ON user_tag.id_tag = tags.id WHERE user_tag.id_user = " + req.session.user_id, function(err, tags)
	  {
	    res.render('edit-profile', {id: req.session.user_id, values: result[0], usertags: tags})
		})
	})
  }
})
...
app.post('/edit-profile-back', function(req, res){
  ...
  con.query('SELECT password, email FROM user WHERE id =' + id, function(err, result){
    if (result[0])
    {
	  ...
	  var reghashtag = /(?<=#)[^\#]*/
	  if((tags = post.tags.match(reghashtag))){
	  tags.forEach(function(element){
        if (element !== "")
        {
...
app.get('/search-back/:page', function(req, res)
{
  ...
  con.query(sql + " LIMIT 12 OFFSET " + ((page * 12) - 12), function (err, result) {
    if (err) throw err
    if (result.length == 0)
      res.render('search', {id: req.session.user_id})
    else{
...
app.post('/inscription-back',function(req,res){
  ...
  if (post.pseudo && post.passwd && post.email && post.prenom && post.nom)
  {
    ...
    if (pseudo.length < 3)
      res.render('inscription', {error: 1})
    else if (pseudo.length > 24)
      res.render('inscription', {error: 2})
    else if (password.length < 6)
      res.render('inscription', {error: 4})
    else if (password.length > 36)
      res.render('inscription', {error: 5})
    else if (!(Regexpassword.test(password)))
      res.render('inscription', {error: 6})
    else if (!(Regexemail.test(email)))
      res.render('inscription', {error: 7})
    else if (prenom.length > 36)
      res.render('inscription', {error: 9})
    else if (nom.length > 36)
      res.render('inscription', {error: 10})
    else {
    //Vérification pseudo email
      var sql = "SELECT count(id) AS nb FROM user WHERE login = '" + pseudo + "'"
      con.query(sql, function (err, result) {
        if (result[0].nb > 0)
          res.render('inscription', {error: 3})
        else {
          var sql = "SELECT count(id) AS nb FROM user WHERE email = '" + email + "'"
          con.query(sql, function (err, result) {
            if (result[0].nb > 0)
              res.render('inscription', {error: 8})
            else {
      ...
      res.render('inscription', {error: 11})
  }
})
  ...
  con.query(sql, function (err, result) {
    ...
    else if (result.length == 1) //SUCCESS
    {
      if (passwordHash.verify(password, result[0].password))
      {
        ...
        res.render('connexion', {error: 22})
    }
    else if (result.length > 1)
    {
      res.render('connexion', {error: 21})
      console.log("⁉️  | Il y a " + result.length + " comptes nommés " + username)
    }
    else if (result.length == 0) // FAIL
    {
      res.render('connexion', {error: 20})
